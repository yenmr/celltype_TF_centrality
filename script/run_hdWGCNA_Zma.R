...
# seurat_obj <- readRDS("SRP145013.rds")
# wgcna_out_file <- "SRP145013_root_wgcna.tsv"
# assay = "SCT"
# DefaultAssay(seurat_obj) <- assay

# Rebuild seurat obj
sct_counts <- SeuratObject::LayerData(
  seurat_obj,
  assay = assay,
  layer = "counts"
)

meta <- seurat_obj@meta.data   # 5868 rows

seurat_clean <- CreateSeuratObject(
  counts   = sct_counts,
  assay    = assay,
  meta.data = meta   # attaches your celltype etc.
)

seurat_clean <- NormalizeData(seurat_clean, verbose = FALSE)
seurat_clean <- FindVariableFeatures(seurat_clean, verbose = FALSE)
seurat_clean <- ScaleData(seurat_clean, verbose = FALSE)
seurat_clean <- RunPCA(seurat_clean, npcs = 50, verbose = FALSE)
seurat_clean <- RunUMAP(seurat_clean, dims = 1:30, verbose = FALSE)
seurat_obj <- seurat_clean
...
hsfB_list <- readr::read_tsv(file.path(repo_root, "resources", "hsfB_Zma.tsv")) %>% mutate(ID = ID_B73v4)
...
# head(seurat_obj@misc$geneName)
# length(seurat_obj@misc$geneName)
# head(seurat_obj@misc$tutorial$wgcna_genes)
# length(seurat_obj@misc$tutorial$wgcna_genes)
# table(seurat_obj@misc$tutorial$wgcna_genes %in% seurat_obj@misc$geneName)
# table(seurat_obj@misc$geneName %in% rownames(seurat_obj))
# table(seurat_obj@misc$tutorial$wgcna_genes %in% rownames(seurat_obj))
...
seurat_obj <- MetacellsByGroups(
  ...
  group.by = c("celltype"), # specify the columns in seurat_obj@meta.data to group by
...
run_wgcna <- function(seurat_obj, celltype){
  ...
  # Try to pick lowest power with SFT.R.sq >= 0.8; fallback to 30
  soft_power_vec <- power_table %>%
    filter(SFT.R.sq >= 0.8) %>%
    pull(Power)

  if (length(soft_power_vec) == 0 || all(is.na(soft_power_vec))) {
    soft_power <- 30
  } else {
    soft_power <- min(soft_power_vec)
  }
