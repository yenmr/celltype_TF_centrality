...
go_gene_ontology_table <- getQueryGOFromAth("Zma")

cytokinin <- read_tsv(gzfile("../resources/Cytokinin-golden-list.tsv.gz"))
zma2ath <- read_tsv(gzfile("../resources/Ath_to_Zma_besthit.tsv.gz"),col_names = FALSE) %>%
  dplyr::mutate(Ath_Gene = purrr::map2_chr(X2, "Ath", ~ gene_from_protein(.x, .y)),
  Zma_Gene = purrr::map2_chr(X1, "Zma", ~ gene_from_protein(.x, .y))) %>% select(Ath_Gene, Zma_Gene)

zma_cytokinin <- zma2ath %>% filter(Ath_Gene %in% cytokinin$ID) %>% pull (Zma_Gene) %>% unique()
cytokinin <- zma_cytokinin

# hsf <- read_tsv("../resources/hsfB_Zma.tsv")
# hsfB <- hsf %>% mutate(ID = as.character(B73v5))

# hsf_name <- setNames(hsfB$Symbol, hsfB$B73v5)

V4ToV5 <- read_tsv(gzfile("../blastp_convert/B73v4_to_B73v5.tsv.gz"), col_names=FALSE) %>% filter(!is.na(X2)) %>%
  mutate(X2 = sub(",.*$", "", X2)) %>%
  #separate_rows(X2, sep = ",") %>% 
  dplyr::rename(V4 = X1, V5 = X2)

hsfB_list <- read_tsv("../resources/hsfB_Zma.tsv") %>% mutate(ID = ID_B73v5)
...
root_filter <- root %>% inner_join(V4ToV5, by = c("gene_name"="V4"),relationship = "many-to-many") %>%
  mutate(gene_name = V5) %>% select(gene_name, module, degree, weighted_degree, celltype) %>% distinct()
...
tf <- zma2ath %>% inner_join(tf, by = c("Ath_Gene" = "Gene_ID")) %>%
  rename(Gene_ID = Zma_Gene) %>% select(Gene_ID, TF_family)
tf_info <- root_filter %>%
