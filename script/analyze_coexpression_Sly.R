...
#convert_id <- RiceIDConvert('Os07g0640900','RAP',toType = 'MSU')
args <- commandArgs(trailingOnly = TRUE)
project <- args[1] # "Mpo_snRNA7d_tak1"
setwd(project)

source("../lib/lib_ortho_GO.R")
go_gene_ontology_table <- getQueryGOFromAth("Sly")

cytokinin <- read_tsv(gzfile("../resources/Cytokinin-golden-list.tsv.gz"))
sly2ath <- read_tsv(gzfile("../resources/Ath_to_Sly_besthit.tsv.gz"),col_names = FALSE) %>%
  dplyr::mutate(Ath_Gene = purrr::map2_chr(X2, "Ath", ~ gene_from_protein(.x, .y)),
  Sly_Gene = purrr::map2_chr(X1, "Sly", ~ gene_from_protein(.x, .y))) %>% select(Ath_Gene, Sly_Gene)

sly_cytokinin <- sly2ath %>% filter(Ath_Gene %in% cytokinin$ID) %>% pull (Sly_Gene) %>% unique()
cytokinin <- sly_cytokinin

# hsf <- read_tsv("../resources/hsfB_Sly.tsv")
# hsfB <- hsf %>% mutate(ID = as.character(ITAG5))

# hsf_name <- setNames(hsfB$Gene_name, hsfB$ID)
hsfB_list <- read_tsv("../resources/hsfB_Sly.tsv") %>% mutate(ID = ID_ITAG5)
hsfB <- hsfB_list %>% pull(ID) %>% unique()
hsf_name <- setNames(hsfB_list$Symbol, hsfB_list$ID)


convert4To5 <- read_tsv(gzfile("../blastp_convert/ITAG4_to_ITAG5_besthit.tsv.gz"), ,col_names = FALSE) %>%
  dplyr::mutate(ITAG5 = purrr::map2_chr(X2, "Sly", ~ gene_from_protein(.x, .y)),
  ITAG4 = sub("\\..*$", "", X1)) %>% select(ITAG5, ITAG4)
...
root_filter <- root %>% inner_join(convert4To5, by = c("gene_name"="ITAG4")) %>%
  mutate(gene_name = ITAG5) %>% select(-ITAG5) %>%
  distinct(gene_name, module, celltype, .keep_all = TRUE)
...
tf <- sly2ath %>% inner_join(tf, by = c("Ath_Gene" = "Gene_ID")) %>%
  rename(Gene_ID = Sly_Gene) %>% select(Gene_ID, TF_family)
tf_info <- root_filter %>%
