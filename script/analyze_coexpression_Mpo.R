...
go_gene_ontology_table <- getQueryGOFromAth("Mpo")

cytokinin <- read_tsv(gzfile("../resources/Cytokinin-golden-list.tsv.gz"))
mpo2ath <- read_tsv(gzfile("../resources/Ath_to_Mpo_besthit.tsv.gz"),col_names = FALSE) %>%
  dplyr::mutate(Ath_Gene = purrr::map2_chr(X2, "Ath", ~ gene_from_protein(.x, .y)),
  Mpo_Gene = purrr::map2_chr(X1, "Mpo", ~ gene_from_protein(.x, .y))) %>% select(Ath_Gene, Mpo_Gene)

mpo_cytokinin <- mpo2ath %>% filter(Ath_Gene %in% cytokinin$ID) %>% pull (Mpo_Gene) %>% unique()
cytokinin <- mpo_cytokinin

hsfB_list <- read_tsv("../resources/hsfB_Mpo.tsv")# %>% mutate(ID = ID_RAP)
...
# root_filter <- root %>% inner_join(convert4To5, by = c("gene_name"="ITAG4")) %>% mutate(gene_name = ITAG5)
root_filter <- root
prefix <- project
...
tf <- mpo2ath %>% inner_join(tf, by = c("Ath_Gene" = "Gene_ID")) %>%
  rename(Gene_ID = Mpo_Gene) %>% select(Gene_ID, TF_family)
tf_info <- root_filter %>%
